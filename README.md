...
- 🛠️ **全新现代化设置中心 (Settings)**：
  ...
  - **应用管理**：全局活动应用快照同步对齐、子序列模糊搜索（Fuzzy Search）、在岛内隐藏/展示开关、手动添加 Bundle ID 与一键清空黑名单；
...
1. 前往 [GitHub Releases](https://github.com/AeroSheepZ/NotchRail/releases/latest) 下载最新版本（`v0.0.4`）的 `.dmg` 或 `.zip`。
2. 双击打开 `NotchRail-v0.0.4.dmg`，将 **NotchRail** 拖入 **Applications (应用程序)** 文件夹。
