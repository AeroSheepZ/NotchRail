...
- 🎨 **像素级高保真镜像与第 0 帧秒开**：多层级真实图元解析（逐窗位图捕获 → 全局应用图元注册表 `appAssetVault` → 跨周期持久缓存），**零降级、无任何占位符**。后台与快照原子化同步预热，展开灵动岛瞬间**第 0 帧直接呈现高清真实图标**，去除多余常态线框与内边距挤压，还原原生通透观感。
...
- 🖥️ **双屏独立多实例架构与零溢出隐藏**：
  - **双面板物理隔离**：MacBook 物理刘海屏由主面板**永久守护**，紧凑胶囊与黄色溢出徽标始终常驻；外接平直屏由独立副面板接管，折叠常态完全隐形且鼠标物理直通，触碰顶部中央受限热区并停留片刻后原位展开统一的纯黑仿真灵动岛；
  - **零跨屏抢夺**：双屏各持独立状态机与穿透流，聚焦在显示器间移动时 0 撕裂、0 徽标闪烁、0 视觉瞬移，彻底告别「余光小胶囊消失」；
  - **全拓扑自适应**：智能适配 MacBook 刘海、单平直屏（Mac mini / 合盖模式）、双平直外接屏；多屏策略支持聚焦跟随（`followFocusedScreen`）、仅主屏（`mainScreenOnly`）或外接屏禁用；
  - **零溢出自动隐藏**：原生菜单栏无遮挡时自动隐藏胶囊。
- 🖱️ **岛内流体拖拽重排**：展开态下可直接拖拽图标进行流体位移重排，松手后原子持久化自定义顺序（`customItemOrder`），未排序项按物理坐标依序追加；**不拦截右键 / Control Click**，第三方应用原生上下文菜单与 `CGEvent.postToPid` 派发机制 100% 保持完整。
