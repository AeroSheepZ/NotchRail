...
- 🚀 **毫秒级极速扫描 (< 15ms)**：通过 macOS SkyLight 窗口系统、单次批量窗口描述提取与非阻塞异步辅助功能架构，毫秒级精准扫描状态栏窗口层级（`kCGStatusWindowLevel`），零 UI 卡顿。
- 🔋 **能耗与发热根治 (Smart Heartbeat)**：菜单栏扫描按三档节拍运行 —— `Dormant`（完全休眠，定时器彻底挂起，空闲 CPU 0.0%）、`Armed`（光标靠近顶边热区时单次增量预热，展开第 0 帧位图即命中）、`Active`（展开态 2.0s 心跳，收起后 1.5s 冷却回归休眠）；窗口指纹未变的静态图标**不再重复创建 `CGContext` 与像素扫描**；全局鼠标移动事件节流至 16ms（60Hz），并在屏幕中下部无关区域 0 开销熔断。
...
- 🖥️ **双屏独立多实例架构与零溢出隐藏**：
  - **双面板物理隔离**：MacBook 物理刘海屏由主面板**永久守护**，紧凑胶囊与黄色溢出徽标始终常驻；外接平直屏由独立副面板接管，折叠常态 100% 隐形（`alpha = 0`）且鼠标物理直通，触碰顶部中央 240pt 热区 120ms 后原位展开统一的纯黑仿真灵动岛；
...
│   │   ├── MenuBar/                # 批量窗口扫描 (< 15ms)、双轨溢出判定、事件驱动 AX 与点击分发
│   │   ├── Island/                 # 灵动岛 SwiftUI 视图、流体拖拽重排、物理弹簧动画与状态机
│   │   ├── Screen/                 # 屏幕几何尺寸、多屏焦点追踪与物理刘海检测
│   │   ├── Window/                 # 双面板 NSPanel 视口管理（主屏常驻 / 副屏隐形穿透）
│   │   ├── Persistence/            # UserPreferences 模型（含 customItemOrder）与 PreferenceStore 持久化
