...
- **多轨物理自律与单前台活动屏独占 (Per-Display Focus & Exclusive Menu Bar)**：
  ...
  - **交互驱动的活动权移交 (Focus Handoff)**：悬停展开保持纯视觉原位呈现（零抢焦）；用户点击某屏灵动岛交互时，系统必须先无感将活动菜单栏移交给该屏（`FocusHandoff`）。该屏升级为活动屏后，其窗口在 WindowServer 登记为 `Item-0`，`AXExtrasMenuBar` 条目池全面就位，状态项获得真实光栅化与点击派发响应；
