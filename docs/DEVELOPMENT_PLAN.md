...
| **v0.0.9：低功耗零发热、双屏独立多实例与岛内管理** | Smart Heartbeat 三档休眠状态机、双轨物理自闭环 + 全局应用图元注册表（替代跨屏成对路由）（**该注册表已于 v0.0.10 废除，见 ADR 0013**）、批量 IPC 抓取、双屏独立常驻面板、灵动岛内流体拖拽重排与偏好设置集中管理 | ✅ **已闭环** |
| **v0.0.10：多屏视口注册表化、按屏独立取图、排序作用域与状态项身份修复** | 废除双槽位面板模型改为按 `displayID` 注册（屏幕数不设上限）、**废除跨屏图元共享层并改为按屏独立取图（区域级合成截图，取代逐窗截图）**、**图元缓存键纳入 `displayID`**、**排序偏好默认按屏独立（多屏共用须用户显式开启）**、**设置项与实现对齐（删除多显示器策略的等价档与「点击图标后自动收起」开关，固化「无遮挡时静默」语义，见 ADR 0015；**胶囊点击改为展开/收起切换、三档唤出语义收敛为唯一定义，见 ADR 0016**）**、状态项身份解析与点击派发目标修复（恒投窗口 owner）与水平中心配对、辅助点击改走会话事件流 + 目标窗口字段路由、设置窗口启动路径根治、文档唯一归属与一致性闸门 | 🚧 **进行中（未闭环 · 未发布）** |
| **v0.0.11+：空间拓展与交互增强** | 全局快捷键调用呼出、触控板手势扩展与高级诊断 | 📋 **规划中** |
