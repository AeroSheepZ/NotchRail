...
| **v0.0.11：多屏隔离收尾（按屏独立取图与排序作用域）** | **决议已立、实现待办（承接 v0.0.10 移出项）**：废除跨屏图元共享层并改为按屏独立取图（区域级合成截图，取代逐窗截图，见 [ADR 0013](adr/0013-per-display-icon-capture.md)）、图元缓存键纳入 `displayID`、排序偏好默认按屏独立（多屏共用须用户显式开启，见 [ADR 0014](adr/0014-per-display-item-order.md)）、随共享层废除移除 `storeKeepingHighestResolution` 等保护性补丁。**卡点**：非活动屏原位派发可达性未取证（ADR 0013 未决项，只能走应用内诊断运行器）、屏幕稳定标识待定案（ADR 0014 决议 5） | 📋 **规划中** |
