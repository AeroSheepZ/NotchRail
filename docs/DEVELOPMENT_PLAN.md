...
| **v1.0.0：全局快捷键呼出、macOS 26 现代设置中心与全链路设置项依赖闭环** | 全局快捷键呼出/收起灵动岛（默认 `⌥ ~`，单前台活动屏独占派发与 0 溢出触觉微动提示）、快捷键录制组件（HotKeyRecorderView）、设置中心全链路选项前后依赖自洽（“仅主屏”禁用跨屏同步、“仅点击”禁用移入延迟）、应用管理状态看板分组（灵动岛展示项 vs 原生可见项）、macOS 26 原生 HIG 卡片式设计与通透材质升级 | ✅ **已闭环** |
| **v1.0.1：Swift 6.4 现代工具链对齐与严格并发隔离** | 升级 Swift Package 工具链声明至 6.4、对齐 Swift 6 Complete Concurrency 严格隔离检查、消除系统 C 全局变量并发拦截、升级 GitHub Actions CI 检出依赖 | ✅ **已闭环** |
| **v1.1.0+：空间拓展与交互增强** | 触控手势实验性探索、自动化高级诊断与脚本扩展 | 📋 **规划中** |
