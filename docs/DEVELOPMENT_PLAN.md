...
| **v0.0.10：多屏视口注册表化、状态项身份与图元修复** | 废除双槽位面板模型改为按 `displayID` 注册（屏幕数不设上限）、状态项图元键逐项唯一、状态项点击派发目标修复（恒投窗口 owner）与水平中心配对、辅助点击改走会话事件流 + 目标窗口字段路由、设置窗口启动路径根治、文档唯一归属与一致性闸门 | 🚧 **进行中（未闭环 · 未发布）** |
