...
| **v0.0.8：平直外接屏双轨纯几何碰撞与视口借调** | 剥离平直屏虚拟刘海、动态 App 菜单碰撞、中央 240pt 防抖热区、吸顶悬浮托轨与视口借调流转 | ✅ **已闭环** |
| **v0.0.9：低功耗零发热、双屏独立多实例与岛内管理** | Smart Heartbeat 休眠状态机、成对窗口位图路由、批量 IPC 抓取、双屏独立常驻面板、灵动岛内流体拖拽重排与偏好设置集中管理 | ✅ **已闭环** |
